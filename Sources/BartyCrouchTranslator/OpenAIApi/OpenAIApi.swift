    ...
    var method: HttpMethod {
        ...
        case let .translate(sources, sourceLanguage, targetLanguage, context, _):
          ...
          let content: String = """
            ...
            // Optional: If you want to supply the comment in the conversation,
            // you can pass it as an additional user message or incorporate it
            // into the system message. Here, we do it explicitly:
           // let commentMessage: OpenAIChatMessage? = comment.map {
            //    OpenAIChatMessage(role: "user", content: "(Comment: \($0))")
          //  }
            
            // Collect messages. We'll place system first, then user text,
            // and optionally the comment. The sequence can be adapted as needed.
            var messages = [systemMessage, userMessage]
           // if let commentMsg = commentMessage { messages.append(commentMsg) }
